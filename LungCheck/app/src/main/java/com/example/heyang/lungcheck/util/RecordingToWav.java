...
public class RecordingToWav {
    public static RecordingToWav getInstance() {
        RecordingToWav result;
        int CAPTURE_SAMPLE_RATE_HZ = 8000;
        do {
            result = new RecordingToWav(
                    true,
                    AudioSource.DEFAULT,
                    CAPTURE_SAMPLE_RATE_HZ,
                    AudioFormat.CHANNEL_IN_MONO,
                    AudioFormat.ENCODING_PCM_16BIT);
        } while (!(result.getState() == RecordingToWav.State.INITIALIZING));
        return result;
    }
    ...
    public AudioRecord.OnRecordPositionUpdateListener updateListener = new AudioRecord.OnRecordPositionUpdateListener() {
        public void onPeriodicNotification(AudioRecord recorder) {
            ...
            try {
                ...
                Log.e(RecordingToWav.class.getName(),
     ...
     *
     */
    public RecordingToWav(boolean uncompressed, int audioSource,
        ...
        try {
            ...
            if (rUncompressed) { // RECORDING_UNCOMPRESSED
                ...
                if (bufferSize < AudioRecord.getMinBufferSize(sampleRate,
                    ...
                    Log.w(RecordingToWav.class.getName(),
            ...
            if (e.getMessage() != null) {
                Log.e(RecordingToWav.class.getName(), e.getMessage());
            } else {
                Log.e(RecordingToWav.class.getName(),
    ...
    public void stop() {
        if (state == State.RECORDING) {
            ...
            try {
                ...
                Log.e(RecordingToWav.class.getName(), "I/O exception occured while closing output file");
                state = State.ERROR;
            }

            state = State.STOPPED;
        } else {
            Log.e(RecordingToWav.class.getName(), "stop() called on illegal state");
            state = State.ERROR;
        }
    }
